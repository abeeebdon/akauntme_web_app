...
import {
    LogoIcon, DashboardIcon, SalesIcon, DebtsIcon, StocksIcon, ServicesIcon, ExpensesIcon,
    ReportsIcon, HistoryIcon, SettingsIcon, LogoutIcon, MarketplaceIcon
...
const MainLayout: React.FC<MainLayoutProps> = ({ children, className }) => {
    ...
    return (
        <div className={`flex w-full overflow-hidden ${className}`}>
            <div className={`relative flex h-screen ${isExpanded ? "w-[14%]" : "w-[5%]"} hidden lg:block transition-width duration-300 bg-akauntme`
            }>
                <div className="flex flex-col justify-between h-full p-4">
                    <div>
                        ...
                        <ul className="mt-10 flex flex-col h-[70vh] justify-around">
                            {
                                Links.map((e, index) => {
                                    return (
                                        ...
                                        >
                                            <Link href={e.link} className={`flex items-center gap-x-2  ${isExpanded ? "" : "justify-center"} rounded-md`}>
                                                {isExpanded ?
                                                    <div className='hidden lg:block bg-transparent hover:bg-white rounded-md text-white hover:text-akauntme p-1'>{e.icon}</div>
                                                    :

                                                    <Tooltip text={`${e.title}`} bg="akauntme" className="rounded-md *:hover:bg-white *:hover:text-akauntme" position="">
                        ...
                        <div className="mt-16">
                            ...
                            >
                                <Link href="#" className={`flex items-center gap-x-[-10px] ${isExpanded ? "" : "justify-center"} *:hover:bg-white *:hover:text-akauntme `}>
            ...
            <div className={` ${isExpanded ? 'w-full lg:w-[86%]' : 'w-full lg:w-[95%]'} bg-white overflow-y-auto h-screen overflow-x-hidden`}>
                <div className="mb-20 px-3 pt-4 lg:px-5 dark:bg-akauntme-dark-2 text-gray-700 dark:text-neutral-400">
                    {children}
                </div>
